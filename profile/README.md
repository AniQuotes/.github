<p align="center"><a href="https://github.com/Shineii86/AniQuotesAPI"><img src="https://github.com/Shineii86/AniQuotesAPI/blob/main/assets/LogoSD.png" LOGO" width="200" height="200"/></a></p>

<div align="center"> <sup>
    
[Creator](https://github.com/Shineii86) • [API](https://github.com/AniQuotes/API) • [Privacy Policy](https://github.com/AniQuotes/PrivacyPolicy) • [Terms of Service](https://github.com/AniQuotes/ToS)

</sup>
</div>

## About Us
Ani Quotes is dedicated to providing free, high-quality anime quote APIs to developers worldwide. Our mission is to build tools that inspire creativity and connect the global anime community through the power of memorable quotes.

## Core Values:  
- ✨ Authenticity
- 🌍 Accessibility
- 🚀 Innovation
- 💖 Community

## Featured Projects

### Anime Quotes API
![GitHub stars](https://img.shields.io/github/stars/Shineii86/AniQuotesAPI?style=for-the-badge)
![API Status](https://img.shields.io/website?down_color=lightgrey&down_message=offline&label=API%20Status&style=for-the-badge&up_color=green&up_message=online&url=https%3A%2F%2Fanistars.vercel.app%2Fv1%2Frandom)

A RESTful API serving anime quotes with multi-language support and image generation capabilities.

```bash
# Get random quote
curl https://aniquotes.vercel.app/v1/random
```

[Explore Documentation](https://github.com/AniQuotes/Documentation) • [Try Live Demo](https://aniquotes.vercel.app/v1/random)

---

## Our Team

| Role | Members |
|------|---------|
| **Core Developers** | [@Shineii86](https://github.com/Shineii86) |
| **Content Curators** |  |
| **Community Managers** |  |
| **Community Bots** | [@QuinxBot](https://github.com/QuinxBot) |

---

## Join Our Community

[![Telegram](https://img.shields.io/badge/Join-Telegram-blue?style=for-the-badge&logo=data:image/svg%2bxml;base64,)](https://telegram.me/AniQuotesAPI)

**Monthly Events**:
- [x] 🎤 API Workshop (1st Sunday)
- [ ] <s> 🎨 Design Challenge (Every Friday) </s>
- [ ] <s> 🌐 Translation Jam (Monthly) </s>

---

## Support Our Mission

Help us keep our services free and accessible to all:

[![Sponsor](https://img.shields.io/badge/Sponsor-%E2%9D%A4%EF%B8%8F-red?style=for-the-badge)](https://github.com/sponsors/AniQuotes)
[![Open Collective](https://img.shields.io/badge/Open_Collective-Support-blue?style=for-the-badge)](https://opencollective.com/aniquotes)

## Contact Us

**General Inquiries**: aniquotes@gmail.com  

---

## License
All projects under AniQuotas organization are released under the [MIT License](https://opensource.org/licenses/MIT).

```
THE ANIME QUOTES ARE PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND.
All anime content belongs to their respective copyright holders.
Our API is for educational and inspirational purposes only.
```

<div align="center">

<sup><b>Copyright © 2025 <a href="https://telegram.me/AniQuotes">AniQuotes</a> Organization All Rights Reserved</b></sup>

<sup> [Privacy Policy](https://telegram.me/AniQuotes/Privacy) • [Terms of Service](https://telegram.me/AniQuotes/ToS) </sup>

</div>
